...
def main(_config, seed):
    ...
    if _config['weights_override'] is not None:
        weights = [_config['weights_override']]
    ...
    if _config['save_log']:
        log_file = f'./results_for_paper/log_seq{test_sequence}.csv'
    ...
    pc_lidar_path = os.path.join(output_dir, 'pointcloud', 'lidar')
    if not os.path.exists(pc_lidar_path):
        os.makedirs(pc_lidar_path)
    pc_input_path = os.path.join(output_dir, 'pointcloud', 'input')
    if not os.path.exists(pc_input_path):
        os.makedirs(pc_input_path)
    pc_pred_path = os.path.join(output_dir, 'pointcloud', 'pred')
    ...
    for batch_idx, sample in enumerate(tqdm(TestImgLoader)):
        if _config['max_eval_batches'] > 0 and batch_idx >= _config['max_eval_batches']:
            break
        N = 100 # 500
        # if batch_idx > 200:
        #    break
